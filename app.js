// PASSWORD SEQUENCE PROGRAM

let password = (enterPassword) => {
  enterPassword= enterPassword.toUpperCase()

  let newPass= enterPassword.split("")

for (let i =0; i<newPass.length-1; i++) {

  let firstChar=newPass[i]

  let nextChar=newPass[i+1]

  let difference= Math.abs(firstChar.charCodeAt(0) - nextChar.charCodeAt(0))
